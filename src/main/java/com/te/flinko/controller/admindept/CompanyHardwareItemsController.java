...
public class CompanyHardwareItemsController extends BaseConfigController {
	...
	public ResponseEntity<SuccessResponse> updateHardwareItemsEmployeeInfo(
		...
		String message = companyHardwareItemsService.updateHardwareEmployeeInfo(getCompanyId(), companyHardwareItemsDTO);
		return ResponseEntity.status(HttpStatus.OK)
				.body(SuccessResponse.builder().error(false).message(message).data(message)
						.build());
	}
	...
	public ResponseEntity<SuccessResponse> generateIndentification(@RequestBody CompanyHardwareItemsDTO companyHardwareItemsDTO) {
		...
		String message = companyHardwareItemsService.markNotWorking(getCompanyId(), companyHardwareItemsDTO);
		return ResponseEntity.status(HttpStatus.OK)
				.body(SuccessResponse.builder().error(false).message(message)
						.data(message).build());
	...
	@GetMapping("generateNumber/{hardwarePC}")
	public ResponseEntity<SuccessResponse> generateNumber(@PathVariable String hardwarePC) {
		log.info("List of identification number");
		return ResponseEntity.status(HttpStatus.OK).body(SuccessResponse.builder().error(false)
				.message("number").data(companyHardwareItemsService.generateNumber(getCompanyId(),hardwarePC)).build());
	}
