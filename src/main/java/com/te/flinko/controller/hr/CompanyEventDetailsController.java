...
public class CompanyEventDetailsController {
	...
	private static final String DATA_INSERTED = "Details Saved Successfully!!";
...
// Controller for adding the New Event
	...
	ResponseEntity<SuccessResponse> deleteEvent(@PathVariable("eventId") Long eventId) {
		...
		return new ResponseEntity<>(SuccessResponse.builder().data(null).error(false).message("Event deleted successfully").build(),
				HttpStatus.OK);
