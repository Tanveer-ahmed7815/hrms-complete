...
public class TaskServiceImpl implements TaskService {
	...
	public TaskDetailsDTO assignTaskToEmployee(AssignTaskDTO assignTaskDTO) {
		...
		if (projectDetails.getCompanyInfo().getCompanyId().equals(assignTaskDTO.getCompanyId())) {
			...
			notificationServiceImpl.saveNotification(
					"Task " + taskDetailsDTO.getTaskName() + " has been assigned to you",
					personalInfo.getEmployeeInfoId());

			if (personalInfo.getExpoToken() != null) {
				pushNotificationService.pushMessage("Flinko",
						"Task " + taskDetailsDTO.getTaskName() + " has been assigned to you",
						personalInfo.getExpoToken());
			}
	...
	public TaskDetailsDTO unAssignTask(Long companyId, String taskId, Long projectId, String reason) {
		...
		List<EmployeePersonalInfo> employeeList = employeePersonalInfoRepo
				.findByEmployeeOfficialInfoEmployeeIdAndCompanyInfoCompanyId(projectTaskDetails.getAssignedEmployee(),
						companyId);
		...
		if (!employeeList.isEmpty()) {
			notificationServiceImpl.saveNotification(
					"Task " + taskDetailsDTO.getTaskName() + " has been unassigned from you",
					employeeList.get(0).getEmployeeInfoId());

			if (employeeList.get(0).getExpoToken() != null) {
				pushNotificationService.pushMessage("Flinko",
						"Task " + taskDetailsDTO.getTaskName() + " has been unassigned from you",
						employeeList.get(0).getExpoToken());
			}
		}
		return taskDetailsDTO;
	...
	public TaskDetailsDTO assignTask(Long projectId, String taskId, Long companyId, String assignedEmployee) {
		...
		notificationServiceImpl.saveNotification("Task " + taskDetailsDTO.getTaskName() + " has been assigned to you",
				personalInfo.getEmployeeInfoId());

		if (personalInfo.getExpoToken() != null) {
			pushNotificationService.pushMessage("Flinko",
					"Task " + taskDetailsDTO.getTaskName() + " has been assigned to you", personalInfo.getExpoToken());
		}
	...
	public TaskDetailsDTO editTask(TaskDetailsDTO taskDetailsDTO) {
		...
		System.err.println("taskDetailsDTO.getStartDate() " + taskDetailsDTO.getStartDate());
	...
	public List<TaskDetailsDTO> fetchTaskBaseOnId(String employeeId, Long companyId, Long projectId, String status) {
		...
		if (status.equalsIgnoreCase("all")) {
			...
			for (ProjectTaskDetails task : taskDetail) {
				arrayList.add(new TaskDetailsDTO(task.getId(), task.getMileStoneId(), null, task.getSubMilestoneId(),
						null, task.getTaskName(), task.getTaskDescription(), task.getCreatedBy(), null,
						task.getCreatedDate(), task.getStartDate(), task.getEndDate(), task.getAssignedDate(),
						task.getStatus(), task.getRemarks() == null ? null : task.getRemarks(), task.getCompletedDate(),
		...
		for (ProjectTaskDetails taskInfo : taskDetail) {
			arrayList.add(new TaskDetailsDTO(taskInfo.getId(), taskInfo.getMileStoneId(), null,
					taskInfo.getSubMilestoneId(), null, taskInfo.getTaskName(), taskInfo.getTaskDescription(),
					taskInfo.getCreatedBy(), null, taskInfo.getCreatedDate(), taskInfo.getStartDate(),
					taskInfo.getEndDate(), taskInfo.getAssignedDate(), taskInfo.getStatus(),
					taskInfo.getRemarks() == null ? null : taskInfo.getRemarks(), taskInfo.getCompletedDate(),
	...
	@Override
	public List<TaskDetailsDTO> delayedTask(Long projectId, Long companyId, String status, String mileStoneId,
			Long subMilestoneId) {
		List<ProjectDetails> projectInfo = projectDetailsRepository.findByCompanyInfoCompanyIdAndProjectId(companyId,
				projectId);
		if (projectInfo == null || projectInfo.isEmpty()) {
		...
		if (status.toLowerCase().equalsIgnoreCase(STATUS_INPROGRESS)) {
			statusList.add(STATUS_INPROGRESS);
		} else {
			throw new CustomExceptionForHr("Please provide a valid Status");
		}
		...
		ArrayList<TaskDetailsDTO> taskDetail = new ArrayList<>();
		LocalDateTime now = LocalDateTime.now();
		for (ProjectTaskDetails a : findByProjectIdAndStatus) {
			TaskDetailsDTO details = new TaskDetailsDTO();
			BeanUtils.copyProperties(a, details);
			if (now.isAfter(a.getEndDate())) {
				List<EmployeePersonalInfo> employeeDetail = employeePersonalInfoRepo
						.findByCompanyInfoCompanyIdAndEmployeeOfficialInfoEmployeeId(companyId, a.getAssignedEmployee())
						.orElseThrow(() -> new CustomExceptionForHr("Employee details not found"));

				if (!employeeDetail.isEmpty()) {
					details.setAssignedEmployeeName(
							employeeDetail.get(0).getFirstName() + " " + employeeDetail.get(0).getLastName());
				}
		...
		Collections.reverse(taskDetail);
		return taskDetail;
	}

	@Override
	public TaskDetailsDTO updateRemark(TaskDetailsDTO taskDetailsDTO) {
		...
		ProjectTaskDetails projectTaskDetails = findByProjectIdAndId.get(0);
		if (projectTaskDetails.getStatus().equalsIgnoreCase(STATUS_UNASSIGNED)) {
			log.info("Given task is not assigned to anyone");
			throw new CustomExceptionForHr("Given task is not assigned to anyone");
		}
		projectTaskDetails.setRemarks(taskDetailsDTO.getRemarks());
