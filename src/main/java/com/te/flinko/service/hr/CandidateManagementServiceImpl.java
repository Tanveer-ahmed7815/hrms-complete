...
public class CandidateManagementServiceImpl implements CandidateManagementService {
	...
	public CandidateInterviewInfoDTO scheduleInterview(CandidateInterviewInfoDTO candidateInterviewInfodto) {
		...
		if (candidateInterviewInfo1 == null || candidateInterviewInfo1.isEmpty()) {
			...
			EmployeePersonalInfo employeePersonalInfo = employeePersonalInfoRepo
					.findByEmployeeInfoIdAndCompanyInfoCompanyId(candidateInterviewInfodto.getEmployeePersonalInfo(),
							candidateInterviewInfodto.getCompanyId());

			if (employeePersonalInfo == null || employeePersonalInfo == null) {
			...
			notificationServiceImpl.saveNotification(
					"Interview is scheduled for " + candidateInfo.getFirstName() + " " + candidateInfo.getLastName()
							+ " on " + candidateInterviewInfoDto.getInterviewDate(),
	...
	private CandidateInterviewInfoDTO rescheduleRound(CandidateInterviewInfoDTO candidateInfodto,
		...
		EmployeePersonalInfo employeePersonalInfo = employeePersonalInfoRepo
				.findByEmployeeInfoIdAndCompanyInfoCompanyId(candidateInfodto.getEmployeePersonalInfo(),
						candidateInfodto.getCompanyId());
		...
		notificationServiceImpl.saveNotification(
				"Interview is rescheduled for " + candidateData.getFirstName() + " " + candidateData.getLastName()
						+ " on " + candidateInterviewInfoDto.getInterviewDate(),
				employeePersonalInfo.getEmployeeInfoId());
	...
	private CandidateInterviewInfoDTO scheduleNextRound(CandidateInterviewInfoDTO candidateInfodto) {
		...
		sendMailToCandidate(interviewInfo);
		notificationServiceImpl.saveNotification(
				"Interview is scheduled for " + candidateDetails.getFirstName() + " " + candidateDetails.getLastName()
						+ " on " + candidateInterviewInfoDto.getInterviewDate(),
				employeePersonalInfo.getEmployeeInfoId());
		
		if (employeePersonalInfo.getExpoToken() != null) {
			pushNotificationService.pushMessage("Flinko", "Interview is scheduled for " + candidateDetails.getFirstName() + " " + candidateDetails.getLastName()
			+ " on " + candidateInterviewInfoDto.getInterviewDate(),
					employeePersonalInfo.getExpoToken());
			}
		return candidateInterviewInfoDto;
		...
		if (employeePersonalInfo != null) {
			...
			if (employeeOfficialInfo != null && employeeOfficialInfo.getOfficialEmailId() != null) {
				...
				mailDto.setBody("<html>\n" + "<body>\n" + "\n"
						...
						+ interviewInfo.getInterviewId() + "/" + interviewInfo.getCandidateInfo().getCandidateId()
						+ "'>" + "Feedback Form" + "</a>" + "<BR />" + "<BR />" + "Thanks and Regards," + "<BR />"
						+ "Team " + interviewInfo.getCandidateInfo().getCompanyInfo().getCompanyName() + "</body>\n"
						+ "</html>");
