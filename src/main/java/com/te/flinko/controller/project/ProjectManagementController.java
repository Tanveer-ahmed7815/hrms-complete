...
public class ProjectManagementController extends BaseConfigController {
	...
	public ResponseEntity<SuccessResponse> getEmployeesForAllocation(@PathVariable Long projectId) {
		List<EmployeeDropdownDTO> listOfEmployeesInProject = projectManagerService.getEmployeesForAllocation(projectId,
				getCompanyId());
		if (listOfEmployeesInProject == null) {
			return ResponseEntity.ok(SuccessResponse.builder().error(true).message("Data not Found")
					.data(listOfEmployeesInProject).build());
	...
	public ResponseEntity<SuccessResponse> addMilestone(@RequestBody MilestoneDTO milestone) {
		...
		else if(milestone.getMileStoneObjectId()==null) {
			return ResponseEntity.ok(SuccessResponse.builder().error(false)
					.message(ProjectManagementConstants.MILESTONE_ADDED).data(addMilestone).build());
		}
		return ResponseEntity.ok(SuccessResponse.builder().error(false)
				.message("Milestone Updated Successfully").data(addMilestone).build());
	}
	...
	public ResponseEntity<SuccessResponse> addSubMilestone(@RequestBody MilestoneDTO milestone) {
		...
		return ResponseEntity.ok(SuccessResponse.builder().error(false)
				.message(ProjectManagementConstants.SUBMILESTONE_ADDED).data(addMilestone).build());
	}
	...
	public ResponseEntity<SuccessResponse> emmployeeUnmapToProject(@PathVariable String employeeId,
			@PathVariable Long projectId) {

		return ResponseEntity.ok(SuccessResponse.builder().error(false)
				.message(projectManagerService.emmployeeUnmapToProject(employeeId, projectId, getCompanyId())).build());
	}
