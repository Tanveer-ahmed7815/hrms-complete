...
public class EmployeeAdvanceSalaryController extends BaseConfigController {
	...
	public ResponseEntity<SuccessResponse> getPerformanceMonthly(@RequestBody EmployeePerformanceDetailsDTO performanceDto){
		EmployeePerformanceInfoDTO performanceMonthly = service.getPerformanceMonthly(performanceDto,getCompanyId());
		return new ResponseEntity<>(new SuccessResponse(false, "Employee monthly performance Fetched successfully", performanceMonthly),
				HttpStatus.OK);
	}
