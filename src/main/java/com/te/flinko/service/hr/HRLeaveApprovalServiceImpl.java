...
public class HRLeaveApprovalServiceImpl implements HRLeaveApprovalService {
	...
	public String updateLeaveStatus(Long companyId, Long employeeInfoId, Long leaveAppliedId, String employeeId,
		...
		List<String> levels = levelsOfApprovalRepository.findByCompanyInfoCompanyId(companyId)
				.map(LevelsOfApproval::getLeave).orElseThrow(() -> new DataNotFoundException(LEVEL_OF_APPROVAL_NOT_FOUND));
					...
					return Optional.ofNullable(adminApprovedRejectDto.getStatus())
							.filter(status -> status.equals(REJECTED)).map(v -> {
								...
								notificationServiceImpl.saveNotification(
										"Leave which was applied from " + employeeLeaveApplied.getStartDate() + " to "
												+ employeeLeaveApplied.getEndDate() + " is rejected by HR",
								...
								if (employeeLeaveApplied.getEmployeePersonalInfo().getExpoToken() != null) {
									pushNotificationService.pushMessage("Flinko",
											"Leave which was applied from " + employeeLeaveApplied.getStartDate()
													+ " to " + employeeLeaveApplied.getEndDate() + " is rejected by HR",
							...
							.orElseGet(() -> Optional.of(adminApprovedRejectDto.getStatus())
									...
									.map(s -> {
										...
										notificationServiceImpl.saveNotification(
												"Leave which was applied from " + employeeLeaveApplied.getStartDate()
														+ " to " + employeeLeaveApplied.getEndDate()
														+ " is approved by HR",
												employeeLeaveApplied.getEmployeePersonalInfo().getEmployeeInfoId());

										if (employeeLeaveApplied.getEmployeePersonalInfo().getExpoToken() != null) {
											pushNotificationService.pushMessage("Flinko",
													"Leave which was applied from "
															+ employeeLeaveApplied.getStartDate() + " to "
															+ employeeLeaveApplied.getEndDate() + " is approved by HR",
													employeeLeaveApplied.getEmployeePersonalInfo().getExpoToken());
										}
					...
					return EmployeeLeaveInfoDTO.builder().leaveAppliedId(leave.getLeaveAppliedId())
							...
							.isActionRequired(
									status.equalsIgnoreCase(PENDING) ? !(leave.getApprovedBy().keySet().contains(HR))
											: null)
							.reportingManager(reportingManager).pendingAt(pendingAt).rejectedBy(leave.getRejectedBy())
							.rejectionReason(leave.getRejectionReason()).build();
				}).collect(Collectors.toList());
				...
				.filter(Objects::nonNull).map(leave -> {
					...
					return EmployeeLeaveInfoDTO.builder().leaveAppliedId(leave.getLeaveAppliedId())
							...
							.isActionRequired(leave.getStatus().equalsIgnoreCase(PENDING)
									...
									: null)
							.pendingAt(pendingAt).rejectedBy(leave.getRejectedBy())
							.rejectionReason(leave.getRejectionReason()).build();
				}).orElseThrow(() -> new DataNotFoundException(DATA_NOT_FOUND));
