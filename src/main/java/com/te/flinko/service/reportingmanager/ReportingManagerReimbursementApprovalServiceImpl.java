					...
					return optional
							...
							.map(rej -> {
								...
								if (reimbursementInfo.getEmployeePersonalInfo().getExpoToken() != null) {
									pushNotificationService.pushMessage("Flinko","Reimbursement request for the amount "+reimbursementInfo.getAmount()+" is Rejected by Reporting Manager",
							...
							.orElseGet(() -> optional.filter(
									...
									.map(sel -> {
										...
										if (reimbursementInfo.getEmployeePersonalInfo().getExpoToken() != null) {
											pushNotificationService.pushMessage("Flinko","Reimbursement request for the amount "+reimbursementInfo.getAmount()+" is Approved by Reporting Manager",
													reimbursementInfo.getEmployeePersonalInfo().getExpoToken());
											}
